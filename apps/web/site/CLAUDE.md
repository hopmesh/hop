...
- `npm ci && npm run build`. `prebuild` runs `npm run sync`, which copies `../../../sim` into `public/sim`
  ...
  relative paths are 2 levels up because the site is at `apps/web/site`; fix them if it moves.
...
- Deploy: `.github/workflows/pages.yml` rebuilds the sim wasm from `core/hop-wasm` at HEAD, runs the
  build, and publishes `apps/web/site/dist` to GitHub Pages. The custom domain is carried by `public/CNAME`.
