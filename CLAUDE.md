...
- **No em-dashes or en-dashes anywhere** (code, comments, docs, commits, PRs). `tools/docs-token-guard.sh` enforces it in CI over docs/site copy AND the source trees, including encoded and lookalike forms. The wire-manifest files (`core/hop-core/vectors/wire-source-manifest.txt`) are the one carve-out: editing one forces a `BUNDLE_VERSION` bump, so clean them during the next real wire bump.
