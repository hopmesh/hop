...
- **Deploy authority lives here.** Every production deployment workflow originates in `hopmesh/hop`; the legacy private monorepo is frozen with no Actions or WIF authority pending archive, and `hopmesh/platform` is pinned private source storage only. Public runtime and bootstrap OpenTofu live under `infra/`. Runtime deploy builds public images before checking out the exact platform commit in `infra/private-source.lock`, then labels every Cloud Run service with both source SHAs and reads them back. The relay fleet is currently OFF (`relays_enabled=false`). The marketing site continues to deploy from `pages.yml` to hopme.sh.
