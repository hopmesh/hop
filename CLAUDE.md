...
- CI (`.github/workflows/ci.yml`) is the gate: 21 jobs. Most are path-filtered off `changes`, so a job that skips still has to report. The aggregate `CI gate` depends on the other 20 and is the ONE required context on `main` (branch-protected); `tools/check-required-checks.sh` keeps that list honest. Adding a job means updating both the `changes` outputs and `gate.needs`.
