...
- **Deploy = merge to main.** GitHub is change management (the PR merge is the only human gate); GCP is plumbing. A merge that goes green triggers `.github/workflows/runtime-deploy.yml`, which builds + pushes the images and applies the runtime root via WIF as `hop-deploy`. See `infra/CLAUDE.md`. The relay fleet is currently OFF (`relays_enabled=false`).
