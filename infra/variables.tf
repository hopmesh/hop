...
variable "private_source_sha" {
  description = "Exact hopmesh/platform source revision staged into the runtime build."
  type        = string

  validation {
    condition     = can(regex("^[0-9a-f]{40}$", var.private_source_sha))
    error_message = "private_source_sha must be exactly 40 lowercase hexadecimal characters."
  }
}

variable "billing_price_ids_version" {
  description = "Pinned numeric version of hop-billing-price-ids used by the runtime apply."
  type        = string

  validation {
    condition     = can(regex("^[1-9][0-9]*$", var.billing_price_ids_version))
    error_message = "billing_price_ids_version must be a positive numeric secret version."
  }
}
