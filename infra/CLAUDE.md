...
- `infra/bootstrap/` is applied by manual dispatch of
  ...
  IAM, service accounts, secrets, Firestore policy, Artifact Registry, the billing
  and runtime Workload Identity Federation pool + provider, and state authority.
...
over WIF, builds and pushes the `hop-relayd` and `hop-example` images, resolves each
to an immutable `@sha256` digest, and runs `tofu apply` on the runtime root with
digest-only image references. A supersession guard reads the current `main` tip
first: if a newer commit already landed, the run SKIPS every remaining step (it
succeeds and applies nothing) so an older tree can never overwrite a newer one.
...
and `serviceAccountUser` on the two runtime identities) plus `storage.objectUser`
scoped to the runtime state prefix. It has no administrative authority: no project,
service-account, or secret IAM admin, no API enablement, no role mutation, and no
access to the bootstrap or billing state.
...
- Runtime images use only `@sha256:` digests; the runtime `relay_image` / `example_image`
  variables reject anything else. The deploy workflow resolves the digests from the
  images it just pushed.
