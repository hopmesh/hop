...
const ROOT = '/Users/jwaldrip/dev/src/github.com/jwaldrip/hop/ble-lab'
...
const ANDROID = `Read the canonical spec at ${ROOT}/SPEC.md (Read tool) in full. Implement the COMPLETE minimal Android (Kotlin) dual-role BLE app under ${ROOT}/android exactly as the spec dictates. It must be a fresh, standalone, buildable project with a NEW package id (e.g. net.waldrip.blelab) so it does not collide with any existing app on the device. To avoid fighting the toolchain, model your gradle wrapper / AGP / SDK config on the KNOWN-GOOD project at /Users/jwaldrip/dev/src/github.com/jwaldrip/hop/android/HopDemo (copy/adapt its gradle infra; do NOT import its app source). Use JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home for gradle. The app must: run the symmetric dual role, log every state transition with a clear grep-able tag, and emit the proof-of-pipe traffic (per-second counter both directions) with a log line when peer bytes advance. ITERATE until \`./gradlew :app:assembleDebug\` (or your module) actually succeeds — do not report built:true unless it truly compiled. Return the build status object.`

const APPLE = `Read the canonical spec at ${ROOT}/SPEC.md (Read tool) in full. Implement the COMPLETE minimal Apple (Swift / CoreBluetooth) dual-role BLE program under ${ROOT}/apple exactly as the spec dictates, with a NEW bundle/service scheme that does not collide with existing apps. PRIMARY build target: a native macOS CoreBluetooth command-line program that compiles with swiftc and logs to stdout (model the swiftc invocation on the known-good /Users/jwaldrip/dev/src/github.com/jwaldrip/hop/apple/hopmac/build.sh, but this PoC needs NO Rust/hop-ffi — pure CoreBluetooth). Structure the BLE logic in a SHARED core .swift file so it can later be wrapped in a minimal iOS app target with no logic changes; provide a build script. The program must run the symmetric dual role, log every state transition with a clear grep-able prefix, and emit the proof-of-pipe traffic (per-second counter both directions) with a log line when peer bytes advance. ITERATE until the macOS build script actually produces a runnable binary — do not report built:true unless it truly compiled. Return the build status object.`
