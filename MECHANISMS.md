...
| **Link encryption** | per-link **Noise NN** channel — *anonymous*: ephemeral-only, so a neighbour learns nothing about you by connecting (confidential vs. passive sniffers). Identity is **opt-in**: a signed `LinkIdentity` sent over the channel (bound to the Noise handshake hash, so a MITM can't relay it) reveals your address to peers who want to be known | wire `LinkPacket::{Handshake,Data}`, `Wire::Identity` |
...
3. **Anonymous links (Noise XX → NN)** — DONE. The link handshake is now ephemeral-only, so a neighbour no longer learns your address just by connecting. `Established.peer` is optional; the direct-delivery shortcut + `peer_links` attribution are best-effort (keyed on the opt-in identity). An **opt-in signed `LinkIdentity`** sent over the channel — signed over the Noise handshake hash so a MITM running separate handshakes with each side can't relay it — re-enables direct delivery/attribution for peers who want to be known. Debug default sends it; **private mode now also stops sending it** (`set_reveal_identity(false)`), so a private device is anonymous even to direct neighbours. New `link_count()` (live links incl. anonymous) drives the "can I hand this off?" UI so a message never shows "Awaiting peers" while a link is up. E2E ratchet/messaging untouched.
