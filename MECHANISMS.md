...
| **Messaging** | sealed device-to-device content + delivery receipt. A **private** `Ack` carries a recipient-only CDH `proof` = `recognition_shared(recipient_spk_secret, orig.ephemeral)`; the sender flips to Delivered only when `recognition_tag_from_shared(proof, for_bundle_id)` equals the original private tag, so an address-knower cannot forge a Delivered (core-protocol-r2-04). `None` on the identity-signed traced ACK. | `PeerMessage`, `Ack` |
