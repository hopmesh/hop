...
| **Routing** | binary spray-and-wait: epidemic flood with copy budgets, custody (forward-before-evict), dedup (`seen`), delivery-ACK vaccine. The vaccine rides `Destination::Vaccine(delivered_id, recognition_token)`: a self-verifying flood (id = `blake3("hop vaccine id v1"‖delivered‖token)`, no src/dst/sig) that lets holders confirm-and-drop a delivered bundle (§39) | `Destination::{Device,AckTo,Broadcast,Vaccine}` |
