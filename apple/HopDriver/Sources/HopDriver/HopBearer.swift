...
public final class HopBearer: NSObject, ObservableObject {
    ...
    public init(config: Config) {
        ...
        self.node = HopNode.open(dbPath: config.dbPath, secret: config.deviceSeed,
    ...
    public func start(name: String) {
        ...
        if isFull {
            ...
            #if os(iOS)
            // iBeacon region monitoring → background/killed wake (§22). iOS-only (CLBeaconRegion).
    ...
    private func sendBundle(dst: Data, contentType: String, body: Data, messageId: UUID) {
        core.async { [weak self] in
            guard let self else { return }
            let id = try? self.node.sendMessage(dst: dst, contentType: contentType,
                                                body: body, requestAck: true)
            DispatchQueue.main.async {
                if let i = self.messages.firstIndex(where: { $0.id == messageId }) {
                    self.messages[i].bundleId = id
                }
                self.pump()
            }
        }
    ...
    private func addLanLink(_ conn: NWConnection, initiator: Bool, name: String?) -> UInt64 {
        ...
        // LanLink runs its I/O on a background queue; node calls funnel to `core`, bookkeeping to main.
        let link = LanLink(id: id, conn: conn,
            onReady: { [weak self] lid in
                DispatchQueue.main.async {
                    ...
                    self.linkUp(lid, initiator: init0)
                }
            },
            onBytes: { [weak self] lid, data in self?.deliver(link: lid, bytes: data) },
    ...
    /// Register an L2CAP channel as a link. Called on main (from the CB delegate, which hops there).
    /// HopLink's stream callbacks fire on the IOThread; `onBytes` funnels to `core`, `onClose` hops
    /// to main for the `links` dict then tells the node (core).
    ...
    /// Route outgoing packets to their link (main). HopLink.send marshals to the I/O thread; the
    ...
    private func applyAdvertising(beacon: Bool) {
        ...
        p.stopAdvertising()
        #if os(iOS)
        let beaconData = beacon ? beaconRegion.peripheralData(withMeasuredPower: nil) as? [String: Any] : nil
        #else
        let beaconData: [String: Any]? = nil   // CLBeaconRegion is iOS-only
        #endif
        if let data = beaconData {
...
extension HopBearer: CBPeripheralManagerDelegate {
    // NOTE: these CB delegate callbacks arrive on `bleQueue` (off main). Each hops to main, where all
    // bearer bookkeeping + @Published + the GATT-data links live; node access funnels to `core` from
    // there. CB API calls (publish/add/respond/updateValue) are issued from main, which is allowed.
    ...
    public func peripheralManager(_ peripheral: CBPeripheralManager, willRestoreState dict: [String: Any]) {
        DispatchQueue.main.async { [self] in status = "restored (peripheral)" }
    }

    public func peripheralManager(_ p: CBPeripheralManager, didPublishL2CAPChannel psm: CBL2CAPPSM, error: Error?) {
        DispatchQueue.main.async { [self] in
    ...
    public func peripheralManager(_ p: CBPeripheralManager, didOpen channel: CBL2CAPChannel?, error: Error?) {
        DispatchQueue.main.async { [self] in
            if let error {
    ...
    public func peripheralManager(_ p: CBPeripheralManager, central: CBCentral,
        ...
        DispatchQueue.main.async { [self] in _ = peripheralGattLink(for: central) }
    }
    ...
    public func peripheralManager(_ p: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
        DispatchQueue.main.async { [self] in
    ...
    public func peripheralManagerIsReady(toUpdateSubscribers p: CBPeripheralManager) {
        DispatchQueue.main.async { [self] in
    ...
    private func peripheralGattLink(for central: CBCentral) -> GattDataLink {
        ...
        linkUp(id, initiator: false)  // peripheral = Noise responder
        NSLog("HOPLOG GATT-data link UP id=\(id) initiator=false (peripheral)")
        return link
    }

    public func closeGattLink(_ id: UInt64) {
        gattLinks[id]?.close()
        gattLinks[id] = nil
        peripheralBackpressure[id] = nil
...
extension HopBearer: CBCentralManagerDelegate, CBPeripheralDelegate {
    // NOTE: these CB delegate callbacks arrive on `bleQueue` (off main). Each hops to main, where the
    // bearer's bookkeeping dictionaries + @Published live; node access funnels to `core` from there.
    ...
    public func centralManager(_ c: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        // UNBIASED role: a peer is just "a BLE peripheral advertising a PSM" — we don't care if it's
        // iOS, Android, a Pi, or an ESP32. Random-id tiebreaker: only the LOWER id is the central
        // (dials); the higher waits as peripheral. Exactly one L2CAP channel per pair.
    ...
    public func centralManager(_ c: CBCentralManager, didConnect peripheral: CBPeripheral) {
        DispatchQueue.main.async { [self] in
            // GATT-read-first (Ditto mechanism A) — PROVEN to work cross-platform (hopmac, fresh BT):
            // discover the Hop service, read the PSM characteristic, THEN open L2CAP. Opening L2CAP
            // directly from the advert PSM ("pure L2CAP", no GATT) does NOT work to Android — Android's
            // accept() succeeds but CoreBluetooth's openL2CAPChannel fails "Unknown error" (half-open).
            // The GATT read is what primes Android to complete the L2CAP. Full discovery (nil), not a
            // targeted UUID filter, which can stall under load.
            guard !opened.contains(peripheral.identifier) else { recover(peripheral); return }
            status = "reading PSM via GATT…"
            peripheral.discoverServices(nil)
        }
    }

    public func centralManager(_ c: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        DispatchQueue.main.async { [self] in scheduleReconnect(peripheral) }
    }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        DispatchQueue.main.async { [self] in
            if error != nil { return recover(peripheral) }
            // Read the PSM characteristic → didUpdateValueFor opens L2CAP (GATT-read-first, proven path).
    ...
    public func peripheral(_ peripheral: CBPeripheral, didOpen channel: CBL2CAPChannel?, error: Error?) {
        DispatchQueue.main.async { [self] in
            if error != nil || channel == nil {
                ...
                // DON'T retry openL2CAPChannel on the SAME connection: Android's accept() succeeds on
                // every attempt, so each failed retry spawns ANOTHER half-open orphan socket on the peer.
                // Under load those orphans pile up against BT's concurrent-L2CAP-channel cap and make even
                // MORE opens fail ("worked then degraded"). Instead drop the ACL immediately — that kills
                // this central's orphan on the peer at once — and reconnect fresh (recover() backs off).
                // Proven (hopmac, fresh BT, single central): the open succeeds first try when not starved.
                recover(peripheral)
                return
            }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic,
        ...
        DispatchQueue.main.async { [self] in
            ...
            connecting.insert(peripheral.identifier)
            linkUp(id, initiator: true)   // central = Noise initiator
...
extension HopBearer: CLLocationManagerDelegate {
    public func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        ...
        if manager.authorizationStatus == .authorizedAlways || manager.authorizationStatus == .authorizedWhenInUse {
            manager.startMonitoring(for: beaconRegion)   // CLBeaconRegion — iOS-only
        }
        #endif
    }
