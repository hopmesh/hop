...
CI_COVERAGE=(
  ...
  "automation|partial|the commit message guard, the docs token guard, the BLE backoff parity pair, the durable-store data map pair, the repo integrity guard, the package export smoke, the Apple published pin guard self-test, the archive-readiness guard pair and the required-checks guard run here; NOT the native attestation npm tests, the agent output guard, the executable reference guard, the Apple pod trunk publisher self-test, the sync/pages/release/artifact-publication/workflow-secrets guards, or the audit skill tests"
...
step "commit-message-guard self-test" bash tools/commit-message-guard.test.sh
step "commit-message-guard"           bash tools/commit-message-guard.sh
...
step "archive-readiness guard self-test" bash tools/archive-readiness-guard.test.sh
step "archive-readiness guard"           python3 tools/archive-readiness-guard.py
