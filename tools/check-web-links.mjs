...
// straight to hopme.sh with no gate. This walks every built HTML file under web/dist, extracts local
...
//
// Usage: node tools/check-web-links.mjs [distDir]   (default: web/dist)
import { readdirSync, readFileSync, statSync, existsSync } from 'fs';
import { fileURLToPath } from 'url';
import { dirname, join, resolve, posix } from 'path';

const here = dirname(fileURLToPath(import.meta.url));
const DIST = resolve(process.argv[2] || join(here, '..', 'web', 'dist'));

if (!existsSync(DIST)) {
  console.error(`error: dist dir not found: ${DIST} (run \`npm run build\` in web/ first)`);
  process.exit(2);
}
