...
1. Is the fleet even supposed to be on? Check the repository variable in the private `hopmesh/monorepo` repository:
   ```sh
...
3. Which regions are unhealthy? Check Cloud Run in the Google Cloud Console (project `hop-mesh`)
   ...
   the "Relay Cloud Run 5xx/429" alert policy (see `hopmesh/monorepo/infra/observability.tf`).
...
- Identity split-brain: If a new relay identity secret version was created,
  ...
  `hopmesh/monorepo/.github/workflows/runtime-deploy.yml` (via repository variable
  `RELAY_IDENTITY_VERSION`), then trigger deployment. Do not re-seed.
...
- Defective image: The deploy pins container images to the commit SHA. Roll back by
  pushing a revert commit to `main` in `hopmesh/monorepo`, which re-builds and re-applies
  via `runtime-deploy.yml`.
...
2. Inspect the failed job logs in GitHub Actions under `hopmesh/monorepo/.github/workflows/runtime-deploy.yml`.
...
1. **Fleet disable rollback (fastest operational mitigation)**:
   In `hopmesh/monorepo`, set the repository variable `RELAYS_ENABLED` to `false` and re-run
   `runtime-deploy.yml`. This tears down all regional Cloud Run services while keeping the anycast
   IP addresses and DNS records intact.
2. **Code rollback (git revert)**:
   Revert the problematic commit on `main` in `hopmesh/monorepo` and push. This triggers
...
- If alerting was unconfigured and operators were unaware of failures, ensure `ALERT_EMAIL`
  is set in `hopmesh/monorepo` repository variables so future incidents generate pages.
