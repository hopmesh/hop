# HopBearerBle, the BLE transport as a CocoaPods pod. The MODULE it exposes is `HopBearerBle`.
#
# The CocoaPods face of the `HopBearerBle` library product in this directory's Package.swift: CoreBluetooth
# L2CAP plus the GATT-PSM handshake, and the iBeacon background wake. ALL Bluetooth lives here; nothing
# BLE leaks into the SDK, the driver, or the app.
# drivers/apple/HopDriver/Sources/HopDriver/HopBearer.swift line 12 does `import HopBearerBle`, so this
...
raise "HopBearerBle.podspec: no version in the release URL in #{manifest_path}" if version.nil?

Pod::Spec.new do |s|
  s.name = "HopBearerBle"
  s.version = version
  s.summary = "The Hop BLE bearer: CoreBluetooth L2CAP links, the GATT-PSM handshake, and iBeacon wake."
  s.description = <<~DESC
    BleBearer and its central/peripheral cores: advertising and scanning, the GATT-PSM handshake that
    opens an L2CAP channel, the Hop link-frame grammar over that channel, dedup with a deterministic
    tiebreaker, backoff scheduling, and the iOS iBeacon region monitor that wakes the bearer in the
    background. It implements the HopContract Bearer protocol, so the driver's BearerManager surfaces
    its links exactly like every other transport.
  DESC
  ...
  s.source_files = "Sources/HopBearerBle/**/*.swift"
  s.swift_version = "5.9"

  # Security is here for SecRandomCopyBytes in BleBearer.swift, not as boilerplate.
  s.frameworks = "Foundation", "CoreBluetooth", "Security"

  # CoreLocation is iOS ONLY, matching the `#if os(iOS)` guards in BeaconWake.swift and
  # BleBearer+Radio.swift: the iBeacon emission payload and the region monitor exist only on iOS, so
  # declaring the framework for macOS as well would claim a link-time dependency this code does not have
  # there. UIKit is deliberately absent for the same kind of reason: BackgroundAssertion.swift reaches
  # UIApplication behind `#if canImport(UIKit)` and Swift autolinks it, which is how HopDriver.podspec
  # already handles the same import.
