...
- **The §32 `hps://` calls are DECLARED, not wrapped.** `ffi.mjs` binds all eighteen (`hps_register`
  ...
  this wrapper pins is the one that added them and pinning a level while binding nothing is the hollow
  pass PLAT-003 was filed about. There is deliberately NO `HopEndpoint` facade over them: nobody has
  asked to host channels from a Node backend yet, and an unused ergonomic layer is a maintenance
  liability. Reach them through the `./raw` export. Note what a publication IS before you wrap one: a
  single content-key-encrypted, per-writer-signed message flooded ONCE, not one-to-one fan-out and not
  a multicast bundle. Membership, invites and revocation are properties of the topic's key handoff,
  and `hop_hps_rekey` is how a member is revoked.
